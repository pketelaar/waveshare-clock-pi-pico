...
def midPointCircleDraw(x_centre, y_centre, r, c):
    ...
    LCD.pixel(x + x_centre,y + y_centre, c)

    # When radius is zero only a single
    # point be printed
    if (r > 0) :
        LCD.pixel(x + x_centre,-y + y_centre, c)
        LCD.pixel(y + x_centre,x + y_centre, c)
        LCD.pixel(-y + x_centre,x + y_centre, c)
...
def drawClock():
    #midPointCircleDraw(120, 120, 10, (LCD.white))
    n12x = 80
    n12y = 10
    for i in range(12):
        newX,newY = rotate( n12x, n12y, (30*i)+20)
        midPointCircleDraw( newX, newY, 3, (LCD.white))
        
def drawSecond( sec ):
    deg = sec * 6
    n12x = 80
    n12y = 10
    newX,newY = rotate( n12x, n12y, deg+20)
...
def drawHour( hour ):
    ...
    n12y = 12
    newX,newY = rotate( n12x, n12y, deg+20)
    LCD.line(screenCenterX, screenCenterY, newX, newY, (LCD.green))
...
if __name__=='__main__':
    ...
    while(True):
        ...
        LCD.text("RP2040-LCD-1.28",60,30,LCD.white)
        
        hms = "{:02}:{:02}:{:02}".format(hour, minute, second)
        ymd = "{:02}/{:02}/{:04}".format(month, day, year)

        LCD.fill_rect(0,40,240,40,LCD.black)
        LCD.text(hms,90,57,LCD.white)
        LCD.text(ymd,80,67,LCD.white)
        #LCD.text(wd,90,77,LCD.white)
        
        LCD.fill_rect(0,80,120,120,LCD.black)
        ...
        LCD.fill_rect(120,80,120,120,LCD.black)
        ...
        LCD.fill_rect(0,200,240,40,LCD.blue)
